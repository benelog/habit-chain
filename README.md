...
| `PUT /meta` | 공개 페이지 제목·설명 저장 → 설정 안의 폼 조각 |
| `GET /export` | 전체 JSON 내려받기 |
| `GET /@owner/name` | **공개 페이지**. 토큰 없이 읽으므로 공개 DB만 보입니다 |
| `GET /@owner/name/habits` | 공개 페이지의 목록 조각 (읽기 전용) |
...
공개 DB라면 `/@owner/name`이 곧 **그 달력의 공유 주소**입니다. 이 경로의 읽기에는 토큰을 붙이지
않습니다 — 방문자의 설정(LocalStorage)도 읽지 않습니다. 그래서 DoltHub가 공개 DB에만 허락하는
익명 읽기가 그대로 공개 범위가 됩니다. 비공개 DB의 공유 주소는 "읽을 수 없습니다"가 뜹니다.

공개 페이지의 **제목과 설명은 사용자의 DB 안에**(`meta` 표, `title`/`description` 행) 있습니다.
서버에는 사용자별 저장소가 없고, 토큰 없는 공개 읽기가 닿는 곳이 거기뿐이기 때문입니다.
설정의 「공개 페이지」에서 적고, 자기 목록 위와 공개 페이지 상단, 그리고 링크 미리보기(og 태그)에 쓰입니다.

공개 페이지의 껍데기는 예외적으로 **서버가 DoltHub를 한 번 읽고** 그립니다. 제목·설명이
`<head>`의 og 태그에 들어가야 카카오톡·페이스북 미리보기가 살기 때문입니다. 목록은 본편과 같이
htmx 조각으로 따라오고, 방문자의 로컬 날짜만 헤더로 실립니다.
