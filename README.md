...
| 경로 | 하는 일 |
|---|---|
| `GET /` | 갈림길. 저장된 달력이 없으면 설정 안내, 하나면 그 달력으로 이동, 여럿이면 선택 화면 |
...
홈(`/`)은 갈림길입니다: 프로필이 없으면 설정 안내, 하나면 그 달력으로 이동(`location.replace`),
여럿이면 선택 화면이 뜹니다. 옛 버전의 단일 키(`habit-chain.db`/`token`)는 처음 읽을 때 프로필로
